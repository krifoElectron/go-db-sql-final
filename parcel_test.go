...
func TestSetAddress(t *testing.T) {
	...
	parcelFromStore, err := store.Get(id)
	assert.Nil(t, err)
	assert.Equal(t, parcelFromStore.Address, newAddress)
}
...
func TestSetStatus(t *testing.T) {
	...
	parcelFromStore, err := store.Get(id)
	assert.Nil(t, err)
	assert.Equal(t, newStatus, parcelFromStore.Status)
}
...
func TestGetByClient(t *testing.T) {
	...
	parcelsFromStore, err := store.GetByClient(client)

	// убедитесь в отсутствии ошибки
	// убедитесь, что количество полученных посылок совпадает с количеством добавленных
	require.Nil(t, err)
	require.Equal(t, len(parcelsFromStore), len(parcels))
